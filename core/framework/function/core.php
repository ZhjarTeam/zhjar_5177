...
function C($key){
	if (strpos($key,'.')){
		$key = explode('.',$key);
		$value = Zhjar\Core::getConfig($key[0]);
		if (isset($key[2])){
		...
		return Zhjar\Core::getConfig($key);
	}
}
...
function curtime_format($format='Y-m-d H:i:s') {
	$cur_time = time();
 ...
 * @return string json
 */
function errorJson($message = '',$errorcode='' ,$custom = array()) {
	$rtn = array (
			'status' => 0,
			"errcode" => $errorcode,
			'msg' => $message,
 ...
 * @return string json
 */
function successJson($message = '', $custom=array()) {
	$rtn = array (
			'status' => 1,
			'msg' => $message,
			'custom'=>$custom
...
function url_html($act,$op,$param=array()) {
	$url = WEB_SITE_URL."/".$act."/".$op;
	if (!empty($param)) {
