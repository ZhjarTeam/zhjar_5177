...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <h3>活动管理</h3>
      <ul class="tab-base">
        <li><a href="index.php?act=trend&op=index"><span><?php echo $lang['nc_manage'];?></span></a></li>
  ...
  <form id="trend_form" method="post">
    <input type="hidden" name="form_submit" value="ok" />
    <input type="hidden" name="trend_id" value="<?php echo $output['trend_array']['id'];?>" />
    <input type="hidden" name="ref_url" value="<?php echo getReferer();?>" />
    <table class="table tb-type2">
      <tbody>
        <tr class="noborder">
          <td colspan="2" class="required"><label class="validation" for="trend_title">标题:</label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform"><input type="text" value="<?php echo $output['trend_array']['name'];?>" name="trend_title" id="trend_title" class="txt"></td>
        ...
        <tr class="noborder">
          <td class="vatop rowform"><input type="text" value="<?php echo $output['trend_array']['color'];?>" name="trend_color" id="trend_color" class="txt"></td>
        ...
        <tr class="noborder">
          <td class="vatop rowform"><input type="text" value="<?php echo $output['trend_array']['ordering'];?>" name="trend_ordering" id="trend_ordering" class="txt"></td>
        ...
        <tr class="noborder">
          <td class="vatop rowform"><input type="text" value="<?php echo $output['trend_array']['link_url'];?>" name="trend_link" id="trend_link" class="txt"></td>
...
$(document).ready(function(){
    var de_type = <?php echo $output['trend_array']['type'];?>;
    $('#trend_type').find('option').eq(de_type-1).attr('selected','selected');
    var child_type = <?php echo $output['trend_array']['child_type'];?>;
    $('#trend_child_type').find('option').eq(de_type-1).attr('selected','selected');
    // 图片上传
    $('#fileupload').each(function(){
        $(this).fileupload({
            dataType: 'json',
            url: 'index.php?act=trend&op=pic_upload&item_id=<?php echo $output['trend_array']['id'];?>',
...
function insert_editor(file_path){
	KE.appendHtml('trend_description', '<img src="'+ file_path + '" alt="'+ file_path + '">');
}
