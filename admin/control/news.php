...
class newsControl extends SystemControl{
	...
	public function indexOp(){
		 ...
		 * 列表
		 */
		$news_list = $model_news->getList($condition,$page);
		...
		$types_list = $model_types->getList(array('group_type'=>0,'order'=>' ordering desc'));
	...
	public function addOp(){
		...
		if (chksubmit()){
			...
			$obj_validate->validateparam = array(
				array("input"=>$_POST["news_title"], "require"=>"true", "message"=>'标题必填'),
				array("input"=>$_POST["news_short_tit"], "require"=>"true", "message"=>'seo关键词必填'),
				array("input"=>$_POST["news_short_tit"], "require"=>"true", "message"=>'seo摘要必填'),
				array("input"=>$_POST["news_content"], "require"=>"true", "message"=>'内容必填'),
			);
			...
			if ($error != ''){
				...
				$insert_array = array();
				$insert_array['title'] = trim($_POST['news_title']);
				$insert_array['short_tit'] = trim($_POST['news_short_tit']);
				$insert_array['keyword'] = trim($_POST['keyword']);
				$insert_array['color'] = trim($_POST['news_color']);
				$insert_array['content'] = trim($_POST['news_content']);
				$insert_array['ordering'] = trim($_POST['news_ordering']);
				$insert_array['views'] = intval($_POST['news_views']);
				$insert_array['author'] = trim($_POST['news_author']);
				$insert_array['source_url'] = trim($_POST['news_source_url']);
				...
				if ($result){
					...
					showMessage("添加成功",'index.php?act=news&op=index');
				}else {
					showMessage("添加失败");
				}
			}
	...
	public function editOp(){
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				$update_array = array();
				$update_array['id'] = intval($_POST['news_id']);
				$update_array['title'] = trim($_POST['news_title']);
				$update_array['short_tit'] = trim($_POST['news_short_tit']);
				$update_array['keyword'] = trim($_POST['keyword']);
				$update_array['color'] = trim($_POST['news_color']);
				$update_array['content'] = trim($_POST['news_content']);
				$update_array['ordering'] = trim($_POST['news_ordering']);
				$update_array['views'] = intval($_POST['news_views']);
				$update_array['author'] = trim($_POST['news_author']);
				$update_array['source_url'] = trim($_POST['news_source_url']);
				$update_array['input_time'] = curtime_format();
		...
		$news_array = $model_news->getOne(intval($_GET['news_id']));
		if (empty($news_array)){
		...
		$model_upload = Model('upload');
		$condition['upload_type'] = '0';
		$condition['item_id'] = $news_array['id'];
		$file_upload = $model_upload->getUploadList($condition);
		if (is_array($file_upload)){
			foreach ($file_upload as $k => $v){
				$file_upload[$k]['upload_path'] = UPLOAD_SITE_URL.'/'.ATTACH_NEWS.'/'.$file_upload[$k]['file_name'];
		...
		//所选类型
		$chos_ids =array();
		if (!empty($news_array['type_ids'])) {
			$chos_ids = explode(',', $news_array['type_ids']);
		}
